    ...
    var body: some View {
        Button(action: {
            ...
            Image(uiImage: state == .empty ? UIImage() : state == .cross ? cross : nought)
                ...
                .opacity(boardViewModel.isInLoosingCondition(fieldIndex: index) ? 0.1 : 1)
                .rotationEffect(boardViewModel.isInWinningCondition(fieldIndex: index) ? .degrees(360) : .degrees(0))
